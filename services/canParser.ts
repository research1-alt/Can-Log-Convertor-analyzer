...
const CAN_LOG_DATA_REGEX = /^\s*[\d-]+\s*\|\s*(\d+)\s*\|\s*([0-9A-Fa-fxX]+)\s*\|\s*(\d+)\s*\|\s*([0-9A-Fa-f\s]+)\s*$/;
const BUSMASTER_V3_REGEX = /^(\d{1,2}:\d{1,2}:\d{1,2}:\d{4})\s+(Rx|Tx)\s+\d+\s+([0-9A-Fa-fxX]+)\s+([a-zA-Z])\s+(\d+)(?:\s+([0-9A-Fa-f\s]*))?$/;

const parseCanLogDataFormatLine = (line: string): CANMessage | null => {
    const match = line.match(CAN_LOG_DATA_REGEX);
